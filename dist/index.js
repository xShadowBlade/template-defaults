...
var require_graceful_fs = __commonJS({
  "node_modules/graceful-fs/graceful-fs.js"(exports2, module2) {
    ...
    function patch(fs4) {
      ...
      fs4.open = open;
      function open(path3, flags2, mode, cb) {
        if (typeof mode === "function")
          cb = mode, mode = null;
        return go$open(path3, flags2, mode, cb);
        function go$open(path4, flags3, mode2, cb2, startTime) {
          return fs$open(path4, flags3, mode2, function(err, fd) {
            if (err && (err.code === "EMFILE" || err.code === "ENFILE"))
              enqueue([go$open, [path4, flags3, mode2, cb2], err, startTime || Date.now(), Date.now()]);
            else {
              if (typeof cb2 === "function")
                cb2.apply(this, arguments);
            }
...
var distDir = import_node_path.default.join(__filename, "..");
var commonDir = import_node_path.default.join(distDir, "common");
var projectTypes = import_fs_extra.default.readdirSync(import_node_path.default.join(distDir, "templates"));
...
function replaceInFile(filePath, searchValue, replaceValue) {
  ...
  const file = import_fs_extra.default.readFileSync(filePath, "utf-8");
  const newFile = file.replace(searchValue, replaceValue);
  import_fs_extra.default.writeFileSync(filePath, newFile, "utf-8");
}
...
var defaultFlags = Object.fromEntries(flagsList.map((f) => [f.name[0], f.defaultValue || false]));
var [args, flags] = (() => {
  const argsArray = [];
  const flagsRecord = {};
  import_node_process2.argv.forEach((item) => {
    if (/^(-|--)/.exec(item)) {
      item = item.replace(/(-|--)+/g, "");
  ...
  flagsList.forEach((flagData) => {
    ...
    let flagValue = defaultFlags[flagName];
...
function setDefaultPrompt(promptOption) {
...
function copyFiles(projectOptions) {
  const { projectType, projectDir, projectName, projectGitRepo, configureEslint, installDep } = projectOptions;
  const projectDirPath = import_node_path2.default.join(import_node_process3.default.cwd(), projectDir);
  import_fs_extra2.default.copySync(commonDir, projectDirPath);
  const projectTypeTemplateDir = import_node_path2.default.join(distDir, "templates", projectType);
  if (projectTypeTemplateDir) {
    import_fs_extra2.default.copySync(projectTypeTemplateDir, projectDirPath, {
  ...
  const packageJsonPath = import_node_path2.default.join(projectDirPath, "package.json");
  const packageLockJsonPath = import_node_path2.default.join(projectDirPath, "package-lock.json");
  if (!configureEslint || !["y", "yes", "true"].includes(configureEslint)) {
    import_fs_extra2.default.removeSync(import_node_path2.default.join(projectDirPath, "eslint.config.mjs"));
    import_fs_extra2.default.removeSync(import_node_path2.default.join(projectDirPath, ".prettierrc.json"));
    const eslintDevDeps = [
      "@eslint/js",
      "@types/eslint__js",
      "eslint",
 ...
 {4}"${dep}": ".*",`), "");
  ...
  if (installDep && ["y", "yes", "true"].includes(installDep)) {
    ...
    try {
      (0, import_node_child_process.execSync)("npm install", { cwd: projectDirPath, stdio: "inherit" });
    } catch (error) {
...
var promptOptions2 = Object.assign(
...
debugLog("Options:", promptOptions2);
copyFiles(promptOptions2);
